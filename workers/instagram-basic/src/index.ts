...
const handler: ExportedHandler<Env> = {
	async fetch(request, env, ctx) {
		...
		const controller = new Controller(env, ctx);
...
class Controller extends Base {
	...
	constructor(readonly env: Env, readonly ctx: ExecutionContext) {
		super();
	...
	async getCachedMedia(cacheKey: Request, profile: UserProfile, count: number) {
		...
		if (!response) {
			...
			this.ctx.waitUntil(this.refreshUserProfile(profile));
		}
...
class InstagramApi {
	...
	constructor(readonly graphClient: InstagramGraph, readonly clientId: string, readonly clientSecret: string) {
...
class SuccessView implements View {
	constructor(readonly username: string, readonly profileUrl: URL) {}
